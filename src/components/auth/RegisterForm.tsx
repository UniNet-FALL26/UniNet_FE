...
export function RegisterForm() {
  ...
  async function submit() {
    ...
    if (await register({ fullName: values.fullName.trim(), email: normalizeEmail(values.email), password: values.password, confirmPassword: values.confirm })) router.replace('/(tabs)');
  }
  return <AuthScreen compact title="Tạo tài khoản UniNet" subtitle="Bắt đầu hành trình học tập, kết nối và phát triển sự nghiệp của bạn.">
    <View style={authStyles.form}>
      <Input label="Họ và tên *" placeholder="Nguyễn Văn A" value={values.fullName} onChangeText={value => change('fullName', value)} autoCapitalize="words" textContentType="name" leftIcon="person" error={errors.fullName} />
      <Input label="Email *" placeholder="example@gmail.com" value={values.email} onChangeText={value => change('email', value)} keyboardType="email-address" autoCapitalize="none" autoCorrect={false} textContentType="emailAddress" leftIcon="email" error={errors.email} />
      <Input label="Mật khẩu *" placeholder="Ít nhất 8 ký tự" value={values.password} onChangeText={value => change('password', value)} secureTextEntry={!showPassword} textContentType="newPassword" leftIcon="password" passwordVisibility={{ visible: showPassword, onToggle: () => setShowPassword(value => !value) }} error={errors.password} />
      ...
      <Input label="Xác nhận mật khẩu *" placeholder="Nhập lại mật khẩu" value={values.confirm} onChangeText={value => change('confirm', value)} secureTextEntry={!showConfirmPassword} textContentType="newPassword" leftIcon="password" passwordVisibility={{ visible: showConfirmPassword, onToggle: () => setShowConfirmPassword(value => !value) }} error={errors.confirm} />
