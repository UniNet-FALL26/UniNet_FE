...
export function LoginForm() {
  ...
  return <AuthScreen backTo="/(auth)/welcome" title="Chào mừng trở lại" subtitle="Đăng nhập để tiếp tục với UniNet.">
    <View style={authStyles.form}>
      <Input label="Email" placeholder="example@gmail.com" value={email} onChangeText={value => { setEmail(value); setErrors(previous => ({ ...previous, email: undefined })); clearError(); }} keyboardType="email-address" autoCapitalize="none" autoCorrect={false} textContentType="emailAddress" leftIcon="✉" error={errors.email} />
      <Input label="Mật khẩu" placeholder="Nhập mật khẩu" value={password} onChangeText={value => { setPassword(value); setErrors(previous => ({ ...previous, password: undefined })); clearError(); }} secureTextEntry={!showPassword} textContentType="password" leftIcon="⌑" passwordVisibility={{ visible: showPassword, onToggle: () => setShowPassword(value => !value) }} error={errors.password ?? (lastAction === 'login' ? error ?? undefined : undefined)} />
