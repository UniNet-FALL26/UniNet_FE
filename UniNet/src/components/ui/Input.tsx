...
type Props = TextInputProps & { label: string; error?: string; leftIcon?: string; passwordVisibility?: { visible: boolean; onToggle: () => void } };
export function Input({ label, error, leftIcon, passwordVisibility, editable = true, style, ...props }: Props) {
  ...
  return <View style={styles.field}>
    ...
    <View style={[styles.box, focused && styles.focused, !!error && styles.errorBox, !editable && styles.disabled]}>
      {leftIcon && <Text style={[styles.icon, styles.leftIcon]}>{leftIcon}</Text>}
...
const styles = StyleSheet.create({
  ...
  focused: { borderColor: colors.primary }, errorBox: { borderColor: colors.error }, disabled: { opacity: 0.5 }, input: { flex: 1, minHeight: 52, fontSize: 15, color: colors.textPrimary, paddingVertical: 0 }, icon: { color: colors.textMuted, fontSize: 17 }, leftIcon: { marginRight: 10 }, rightIcon: { width: 48, height: 48, alignItems: 'center', justifyContent: 'center', borderRadius: 12, marginRight: -10 }, iconPressed: { backgroundColor: colors.border }, error: { fontSize: 13, color: colors.error },
});
